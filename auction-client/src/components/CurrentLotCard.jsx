  ...
  return (
    <div className="card">
      ...
      <div className="w-full h-2 bg-slate-700 rounded mb-4 overflow-hidden">
        <div className="h-2 bg-accent transition-[width] duration-200" style={{ width: `${pct}%` }} />
      </div>
          ...
          <div className="flex flex-wrap gap-2">
            {(player.characters || []).length
              ? player.characters.map((c, i) => (
                  <span key={i} className="px-3 py-1 rounded-full bg-slate-900 border border-slate-700 text-slate-100 text-sm">{c}</span>
                ))
        ...
        <div className="grid grid-cols-4 gap-2">
          {[10,20,50,100].map(step => (
            <button
              ...
              disabled={disableBid}
              className={`py-2 text-sm font-medium ${disableBid ? 'bg-slate-700 text-slate-500 cursor-not-allowed' : 'primary'}`}>
              +{step}
            </button>
          ))}
        </div>
