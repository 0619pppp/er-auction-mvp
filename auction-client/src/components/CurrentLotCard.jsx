...
export default function CurrentLotCard({ lot, timeLeft, onBidAbs, onStart, onNext, started }) {
  ...
  useEffect(() => {
    if (lot?.highestBid != null) setManual(String((lot.highestBid || 0) + 10))
  }, [lot?.highestBid])
        ...
        <div className="flex flex-wrap gap-2">
          {(player.characters || []).length
            ? player.characters.map((c, i) => (
                <span key={i} className="px-3 py-1 rounded-full bg-slate-800 border border-slate-600 text-slate-100 text-sm">
                  {c}
                </span>
              ))
      ...
      <div className="rounded-xl bg-slate-800 p-4 border border-slate-600">
        ...
        <div className="grid grid-cols-4 gap-2">
          {[10,20,50,100].map(step => (
            <button
              ...
              disabled={preview}
              className={`py-2 text-sm font-medium ${preview ? 'bg-slate-700 text-slate-500 cursor-not-allowed' : 'primary'}`}>
              +{step}
            </button>
          ))}
        </div>
        ...
        <div className="mt-3 flex gap-2">
          <input
            ...
            onKeyDown={e=>{ if (e.key==='Enter' && !preview) submitManual() }}
            className="flex-1"
            placeholder="직접 입력 예: 270"
            disabled={preview}
          />
          <button onClick={submitManual} disabled={preview} className="ghost px-4 py-2 text-sm">
            입력가 입찰
          </button>
        </div>
      </div>
